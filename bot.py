...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message is None:
        return  # تجنب أي استثناء إذا الرسالة فارغة
    ...
    await update.message.reply_text(f"أهلاً وسهلاً {user.first_name} البوت لـ @E2e12 فقط ✨💁")
...
async def webhook(req: Request):
    data = await req.json()
    print("📩 Webhook received:", data)  # للتأكد من وصول التحديثات
    try:
