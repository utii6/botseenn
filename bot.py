...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # السماح التلقائي للمدير
    if user_id == ADMIN_ID:
        add_user(user_id)
        await update.message.reply_text(f"👑😂 أهلاً بك يا المدير {user.first_name}")
    ...
    if not is_allowed(user_id):
        await update.message.reply_text("⚠️💔 أنت غير مسموح لك بالوصول راسل @E2E12")
        return
    ...
    if not subscribed:
        await update.message.reply_text(
            f"⚠️ اشترك يقلبي: {CHANNEL_USERNAME}\nبعد الاشتراك اضغط /start مرة أخرى"
        )
        return

    # رسالة ترحيب للمستخدم العادي
    await update.message.reply_text(f"أهلاً وسهلاً {user.first_name} البوت لـ @e2e12 ✨💁")

    # عرض الخدمات بأزرار شفافة
...
async def ban_user_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❗ استخدام: /ban_user <user_id>")

# ===== Telegram Application =====
telegram_app = ApplicationBuilder().token(TOKEN).build()
asyncio.run(telegram_app.initialize())  # مهم جداً لتفادي خطأ Application.initialize
